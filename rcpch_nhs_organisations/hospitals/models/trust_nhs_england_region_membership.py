...
class TrustNHSEnglandRegionMembership(TimeStampAbstractBaseClass):
    """
    Tracks which NHS England Region a Trust belonged to over time.

    Append-only. Each row records that the Trust was a member of the NHS
    England Region over [valid_from, valid_to). The current membership is
    the row with valid_to IS NULL.

    A trust can move NHS England region on merger. Versioning this
    relationship directly avoids traversing the organisation layer for
    'which region was Trust X under on date Y' queries.
    ...
    def __str__(self) -> str:
        return (
            f"{self.trust.ods_code} → {self.nhs_england_region.region_code} "
